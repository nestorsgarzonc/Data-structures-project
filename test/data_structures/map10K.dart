...
void main() {
  ...
  group("Grupo de pruebas finales Map", () {
    test("Insercion de 10K pares", () {
      Stopwatch stopwatch = new Stopwatch()..start();
      for (int i = 0; i < 10000; i++) {
        mapa.insert('Numero ${i}', i);
      }
      print(
          'Insert para 10k de pares en ${stopwatch.elapsedMicroseconds} microsegundos');
      expect(true, true);
    });
    ...
    test("Cambio del valor de un key", () {
      Stopwatch stopwatch = new Stopwatch()..start();
      mapa.setValue('Numero 4000', 25860);
      print(
    ...
    test("Obtener el valor de un key", () {
      Stopwatch stopwatch = new Stopwatch()..start();
      mapa.getValue('Numero 4000');
      print(
          'Obtener de un valor: ${stopwatch.elapsedMicroseconds} microsegundos');
      expect(true, true);
    });
    test("Borrado de 1 valor", () {
      print(mapa.getValue('Numero 5000'));
      Stopwatch stopwatch = new Stopwatch()..start();
      mapa.remove('Numero 5000');
      print(
          'Delete para 1 valor: ${stopwatch.elapsedMicroseconds} microsegundos');
      expect(true, true);
    });
    test("has", () {
      expect(mapa.hasKey('Numero 5000'), false);
    });
    test("get", () {
      print(mapa.getValue('Numero 4000'));
      expect(mapa.getValue('Numero 4000'), 25860);
    });
    test("size", () {
      print(mapa.size());
      expect(mapa.size(), 9999);
    });
  });
}
